...
interface MallFinance     { nombre: string; costoMensual: number; epcSolarRoofing: number; ventasWaterAnker: number; ganancia: number; pctMeta: number }
interface BoothEvent      { nombre: string; fechaInicio: string; fechaFin: string; dias: number; costo: number; ingreso: number; gananciaNeta: number }
...
interface FinanceData {
  year: number; month: number; monthLabel: string; semesterLabel: string
  hdUnitsReady: boolean; cambaseoComisionUnit: number
  resumen: { ingresoMensual: number; costoMensual: number; gananciaNeta: number; margenPct: number; canales: Canal[] }
  homeDepot: { mensual: HomeDepotTienda[]; semestral: HomeDepotTienda[]; metaSemestre: number; metaPorTienda: number; prorrateoMensual: number }
  malls: MallFinance[]
  booths: BoothEvent[]
...
export default function FinanceDashboard({ initMonth }: { initMonth: string }) {
  ...
  const [applied, setApplied] = useState(initMonth)
  ...
  const [hdPeriodo, setHdPeriodo] = useState<'mensual' | 'semestral'>('mensual')
  ...
  const fetchData = useCallback(async (m: string) => {
    ...
    try {
      const res = await fetch(`/api/finance/summary?month=${m}`)
  ...
  function handleApply() { setApplied(month); fetchData(month) }
  ...
  async function generatePDF() {
    ...
    try {
      ...
      pdf.save(`windmar-finanzas-${applied}.pdf`)
  ...
  return (
    <div className="-mx-4 -mt-6 px-4 pb-16 min-h-screen" style={{ background: PAGE_BG, backgroundAttachment: 'fixed', fontFamily: SANS }}>
      <div ref={contentRef} style={{ maxWidth: 1320, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#AFC3EE' }}>
                Finanzas & ROI de Canales · {data?.monthLabel ?? '—'}
              </span>
            </div>
        ...
        {/* PERIOD BAR */}
        <div data-pdf-ignore="true" style={{ background: '#fff', borderRadius: 20, boxShadow: SHADOW_MD, padding: '22px 28px', marginTop: 28, display: 'flex', alignItems: 'flex-end', gap: 22, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', color: NAVY, marginBottom: 10 }}>Período (mensual)</div>
            <input type="month" value={month} onChange={e => setMonth(e.target.value)} style={{ fontFamily: SANS, fontSize: 15, fontWeight: 600, color: NAVY, border: '1.5px solid #DDE3EE', borderRadius: 12, padding: '10px 14px' }} />
          </div>
          ...
          <>
            ...
            <div id="sec-resumen" style={{ scrollMarginTop: 96, marginTop: 22 }} className="pdf-break">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }} className="max-md:!grid-cols-2">
                <Kpi label="Ingreso Mensual Est." value={money(data.resumen.ingresoMensual)} sub="Generado por canales" accent={BLUE} />
                <Kpi label="Costo / Pago Mensual" value={money(data.resumen.costoMensual)} sub={`Incl. Home Depot ${money(data.homeDepot.prorrateoMensual)}/mes`} accent={NAVY} />
                <Kpi label="Ganancia Neta Mensual" value={money(data.resumen.gananciaNeta)} sub={`Margen ${pct1(data.resumen.margenPct)}`} accent={data.resumen.gananciaNeta >= 0 ? GREEN : RED} valueColor={data.resumen.gananciaNeta >= 0 ? GREEN : RED} />
            ...
            {/* ── HOME DEPOT ── */}
            {(() => {
              const stores = hdPeriodo === 'mensual' ? data.homeDepot.mensual : data.homeDepot.semestral
              const metaStore = hdPeriodo === 'mensual' ? data.homeDepot.metaPorTienda / 6 : data.homeDepot.metaPorTienda
              const metaTotal = hdPeriodo === 'mensual' ? data.homeDepot.metaSemestre / 6 : data.homeDepot.metaSemestre
              ...
              return (
                <SectionCard id="sec-hd" title="Home Depot" right={
                  <div data-pdf-ignore="true" style={{ display: 'flex', gap: 6, background: '#EEF2FA', padding: 4, borderRadius: 999 }}>
                    {(['mensual', 'semestral'] as const).map(p => (
                      <button key={p} onClick={() => setHdPeriodo(p)} style={{ border: 'none', borderRadius: 999, padding: '7px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', textTransform: 'capitalize', color: hdPeriodo === p ? '#fff' : GREY, background: hdPeriodo === p ? NAVY : 'transparent' }}>{p}</button>
                    ))}
                  </div>
                }>
                  {!data.hdUnitsReady && (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, marginBottom: 24 }} className="max-md:!grid-cols-2">
                    <Kpi label={`Generado (${hdPeriodo})`} value={money(data.hdUnitsReady ? totalIngreso : null)} sub={`Meta ${money(metaTotal)}`} accent={BLUE} />
                    <Kpi label="Pago a Home Depot" value={money(metaTotal)} sub={hdPeriodo === 'mensual' ? 'Prorrateo $500K ÷ 6' : '$500K semestral'} accent={NAVY} />
                    <Kpi label="% Cobertura Meta" value={data.hdUnitsReady ? pct1(cobertura) : '—'} sub="Generado ÷ pago" accent={ORANGE} valueColor={data.hdUnitsReady ? semaforo(cobertura).color : undefined} />
                  ...
                  return (
                    <div key={m.nombre}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 13, color: GREY }}>Ganancia <strong style={{ color: sem.color }}>{money(m.ganancia)}</strong> / Costo {money(m.costoMensual)} · <strong style={{ color: sem.color }}>{pct1(m.pctMeta)}</strong></span>
                      </div>
                      <ProgressBar pct={m.pctMeta} color={sem.color} />
                      <div style={{ fontSize: 12, color: FLAT, marginTop: 6 }}>
                        EPC Solar/Roofing {money(m.epcSolarRoofing)} → 15% = {money(m.epcSolarRoofing * 0.15)} · Water/Anker {money(m.ventasWaterAnker)} → 10% = {money(m.ventasWaterAnker * 0.10)}
                      </div>
                    </div>
                  )
                })}
              </div>
              <p style={{ fontSize: 12, color: ORANGE600, marginTop: 18 }}>⚠ Costos mensuales por mall son valores de config (TODO: confirmar los montos reales).</p>
            </SectionCard>
            ...
            <SectionCard id="sec-cambaseo" title="Cambaseo — Coordinadores">
              {(() => {
                ...
                return (
                  <>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, marginBottom: 24 }} className="max-md:!grid-cols-2">
                      <Kpi label="Ventas Solar/Roofing" value={num(totVentas)} sub={`Comisión $${data.cambaseoComisionUnit}/venta este mes`} accent={BLUE} />
                    ...
                    <p style={{ fontSize: 12, color: ORANGE600, marginTop: 16 }}>Ganancia compañía = 15% del EPC por venta de solar/roofing. Comisión: solar/roofing ${data.cambaseoComisionUnit}/venta (por mes) + water/PPS $10/venta. Guagua $500/mes + salario $600/sem.</p>
                  </>
                )
              })()}
            </SectionCard>
            ...
            <div style={{ textAlign: 'center', fontSize: 12, color: FLAT, padding: '18px 0 0' }}>
              Windmar Home · Dashboard de Finanzas & ROI · {data.monthLabel}
            </div>
          </>
        )}
      </div>
